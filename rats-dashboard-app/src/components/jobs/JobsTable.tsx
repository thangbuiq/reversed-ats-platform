...
export default function JobsTable({
    ...
    const passesSearch = React.useCallback((j: JobRecord) => {
    ...
    const sortedAndFiltered = React.useMemo(() => {
        ...
        result.sort((a, b) => {
            const da = a.payload?.date_posted && a.payload.date_posted !== "—" ? new Date(String(a.payload.date_posted)).getTime() : 0;
            const db = b.payload?.date_posted && b.payload.date_posted !== "—" ? new Date(String(b.payload.date_posted)).getTime() : 0;
            return db - da;
        });
    ...
    return (
        <>
            <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                {/* Header */}
                <div className="flex flex-col gap-4 p-5 md:p-6 border-b border-gray-100 dark:border-gray-800">
                    <div>
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="h-10 w-full rounded-lg border border-gray-200 bg-transparent py-2 pl-9 pr-3 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                            />
                        </div>
                    </div>
                </div>
                                ...
                                {isLoading ? (
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={7} className="px-5 py-12 text-center text-gray-400 dark:text-gray-500">
                                            {search.trim() ? "No jobs match your filter." : "No jobs found. Try materializing jobs first."}
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    paginatedJobs.map((job) => {
                                        ...
                                        return (
                                            <TableRow
                                                key={job.point_id}
                                                className={`group transition-all duration-200 cursor-pointer hover:bg-brand-50/40 dark:hover:bg-brand-500/5 ${fresh.isEarlyBird ? "bg-success-50/30 dark:bg-success-500/5" : ""}`}
                                            ...
                                            >
                                                ...
                                                <TableCell className="px-4 py-3 text-gray-600 text-start text-theme-sm dark:text-gray-300 max-w-[160px] truncate">
                                                    {job.company_name ?? "—"}
                                                </TableCell>
                                                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-xs dark:text-gray-400 max-w-[140px] truncate">
                                                    {job.location ?? "—"}
                                                </TableCell>
                                                <TableCell className="px-4 py-3 text-start">
                                                ...
                                                <TableCell className="px-4 py-3 text-start" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                                                    {job.job_url ? (
                                                        <a
                                                            href={job.job_url}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="inline-flex items-center gap-1 text-brand-500 hover:text-brand-600 text-theme-xs font-medium transition-colors"
                                                        >
                                                            Apply
                ...
                {renderLimit < sortedAndFiltered.length && (
                    <div className="flex justify-center p-5 border-t border-gray-100 dark:border-gray-800">
                        <button
                            onClick={() => setRenderLimit(prev => prev + 50)}
                            className="rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
                        >
                            Show More
                        </button>
                    </div>
            ...
            {selectedJob && (
                <div className="fixed inset-0 z-[99999]">
                    ...
                    <div className="fixed inset-0 flex items-center justify-center p-4">
                        <div
                            className="relative w-full max-w-xl rounded-xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900 animate-in zoom-in-95 fade-in"
                            onClick={(e) => e.stopPropagation()}
                        >
                            {/* Close button – top right, inside card */}
                            <button
                                onClick={() => setSelectedJob(null)}
                                className="absolute right-4 top-4 z-10 rounded-md p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors dark:hover:text-gray-200 dark:hover:bg-gray-800"
                            >
                            ...
                            <div className="p-6 pb-4 pr-12">
                                ...
                                <div className="flex flex-wrap gap-2 mt-3">
                                    {getPayloadField(selectedJob, "job_level") !== "—" && (
                                        <Badge size="sm" color="info">{getPayloadField(selectedJob, "job_level")}</Badge>
                                    )}
                                    {getPayloadField(selectedJob, "job_type") !== "—" && (
                                        <Badge size="sm" color="light">{getPayloadField(selectedJob, "job_type")}</Badge>
                                    )}
                            ...
                            <div className="px-6 pb-2 max-h-[50vh] overflow-y-auto">
                                <div className="prose prose-sm dark:prose-invert max-w-none text-gray-700 dark:text-gray-300 [&>p]:mb-3 [&>ul]:mb-3 [&>ol]:mb-3 [&>h1]:mt-5 [&>h2]:mt-4 [&>h3]:mt-3 [&>li]:mb-1 leading-relaxed">
                                    <Markdown>
